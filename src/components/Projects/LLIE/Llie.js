...
const Input = (props) => {
    ...
    const sendData = async (canvas) => {
        ...
        await axios.post(Server + 'api/low-light/', fd, {headers: headers})
            .then(res => {
    ...
    return (
        <div className='container'>
            <h1 className={props.isDarkMode ? 'page-heading dark' : 'page-heading'}>Low-Light Image Enhancement</h1>

            <div id='project-intro' className={props.isDarkMode ? 'project-info dark' : 'project-info'}> 
                <p className='text-center'> This project consists of a Deep learning model which aims to Enhance images captured in low-light envirnoment. We have used a CycleGAN and added a attention module to enhance it's capabilites. It is trained on a custom Dataset which can be found HERE. Model was made using keras and tensorflow libraries.</p> <br/>
            ...
            <div className='upload-box row'>
                <div className='text-center col-12 col-xs-12 col-sm-12 col-md-12 col-lg-5 col-xl-5'>
                    {showImage ? 
                        <div>
                            <img id="img_disp" alt="Upload-preview" className='displayImage' src={imageUrl} ref={imageRef}/>
